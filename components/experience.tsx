  ...
  return (
    <section id="experience" className="py-20 px-4">
      <div className="max-w-7xl mx-auto">
        <h2 className="text-4xl md:text-5xl font-bold mb-4 glow-cyan">Professional Experience</h2>
        ...
        <div className={`grid gap-8 ${showList ? "md:grid-cols-3" : "md:grid-cols-1"}`}>
          {/* Experience List */}
          {showList && (
            <div className="md:col-span-1 flex flex-col gap-4">
              {experiences.map((exp, i) => (
                ...
                >
                  <div className="font-bold text-sm uppercase tracking-widest mb-2">{exp.title}</div>
          ...
          <div className={showList ? "md:col-span-2" : "md:col-span-1"}>
            <div className={`transition-all duration-300 ${isAnimating ? "opacity-0" : "opacity-100"}`}>
              <div className="mb-6 w-full aspect-video rounded overflow-hidden border-glow">
                <img
              ...
              <h3 className="text-3xl font-bold mb-2 text-[rgb(0,217,255)]">{experiences[selected].title}</h3>
              <p className="text-[rgb(255,102,0)] text-sm uppercase tracking-widest mb-4">
                {experiences[selected].date}
              </p>
              <p className="text-[rgb(130,140,160)] leading-relaxed mb-6">{experiences[selected].description}</p>

              <h4 className="text-lg font-bold mb-4 text-[rgb(0,217,255)]">Tanggung Jawab & Pencapaian</h4>
              <ul className="space-y-3">
                {experiences[selected].responsibilities.map((resp, i) => (
                  <li key={i} className="flex gap-3 text-[rgb(130,140,160)]">
